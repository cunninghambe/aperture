...
with a full  structure and reports success — and still emits
no ,  or  headers. Control
experiment: real Chromium sends all three to the same 
endpoint. Aperture sent none.

Two inconsistencies followed from claiming Chrome:

1. Zero client hints while claiming Chrome — binary and trivially checked.
2.  reported only , contradicting the
   UA string.

Since the hints cannot be made to agree on this platform, the rule stated in
this document applies: do not make the claim. **Aperture now presents as
Chromium**, matching what the JS API actually reports, so the string and the
API agree. The absent hints remain a tell, but one unusual value beats a
self-contradiction — detectors mostly check agreement, not values.

The version is never spoofed; it comes from , because
the TLS ClientHello is generated by the real BoringSSL build and would
contradict any claim below the JS layer.

 in  asserts the property, and the
suite fails if a future change reintroduces the contradiction.
