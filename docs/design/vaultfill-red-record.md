...
| HEAD | `57e99b4230887cf4089818b5d8839a8e28886eec` ("docs: vault fill spec — execution path, plus ten defects found around the stub", 2026-08-02 23:27:05 -0400) |
...
**The POST build (part 2, and the acceptance battery).**

| artifact | sha256 | size |
|---|---|---|
| `out/main/index.js` | `25ad709a5835121c59f43fe373f6caba35b10c01e9fd963f1a0314f8e1328e37` | 228462 |
| `out/preload/page.cjs` | `de617d9524eb83c30b28138714578b7bc91c031f74135c6ac2ec97e28b1d7119` | 30361 |

`buildVersion` moved to `027a2ee76e56a543`, which severs every existing bench
store. That is the integrity design working, and §15.4 item 10 below records it
doing so.
...
Runner: `scratchpad/battery.sh` (throwaway). Per-run raw output:
`scratchpad/sab-S*.txt`.

Every live run was `npx electron . --seed-vault --e2e-consent=allow
--e2e-consent-delay-ms=1500` followed by `node bench/guards.mjs <token>` — the
byte-identical fixture, guard and command line as the green run.

| # | sabotage (as applied) | must turn RED | observed |
|---|---|---|---|
| S1 | `src/preload/page.ts`: `if (location.origin !== req.expectedOrigin) {` → `if (false as boolean) {` | G20 | **35/36 · RED — G20**, and only G20 |
| S2 | `src/preload/page.ts`: `if (!(input instanceof HTMLInputElement) \|\| input.type !== 'password') {` → `if (false as boolean) {` | G22 (spec also named G18a) | **35/36 · RED — G22**, and only G22. G18a stayed green — see below |
| S3 | `src/preload/page.ts`: fill validation's `if (!el \|\| !el.isConnected) {` → `if (!el) {` | G21 | **35/36 · RED — G21**, and only G21 |
| S4 | `src/mcp/tools.ts`: `registerNeedles(id, needleValues);` → `void needleValues;` | G19 | **35/36 · RED — G19**, and only G19 |
| S5 | `src/preload/page.ts`: `const VERIFY_DELAY_MS = 250;` → `= 0;` | G18b | **35/36 · RED — G18b**, and only G18b |
| S6 | `src/vault/fillPlan.ts`: `if (passwords.length > 1) {` → `if (false as boolean) {` | G23 | **35/36 · RED — G23**, and only G23 |
| S7 | `src/vault/vault.ts`: `this.lastIssued.set(entryId, counter);` → `void counter;` | G26b | **35/36 · RED — G26b**, and only G26b |
| S8 | `src/vault/vault.ts`: `if (rec.rpId !== rp && !rec.aliases.includes(rp)) {` → `if (false as boolean) {` | G17a | **35/36 · RED — G17a**, and only G17a |
| S9 | `src/main/consent.ts`: one line inserted at the top of the credential branch consulting `grants` | G27b | **G27b stayed GREEN — the pairing does not hold.** Caught instead by `test/consent.test.ts` → *"a live profile grant for the same origin does not cover a credential"*, which went RED. See below |
| S10 | `src/preload/page.ts`: `if (isDisabled(input) \|\| input.readOnly) {` → `if (false as boolean) {` | G25 | **35/36 · RED — G25**, and only G25 |
| S11 | *(added by the builder)* `src/preload/page.ts`: `isDisabled(input)` → `input.disabled` — i.e. §6.2's check, literally | G25b | **36/37 · RED — G25b**, and only G25b. Reply: `filled username and password …`; witness: `pw-match:true` |

Nine of the ten specified sabotages turned exactly their named guard red and
left every other guard green — including, each time, all eighteen of the
pre-existing G1–G15 checks. The tenth (S9) is not live-discriminable and is
recorded below. S11 is the builder's own, and it is the one that found a defect
...
| 3 | `npx vitest run` | **20 files, 500 tests passed** (447 at HEAD; +53 new, every pre-existing test untouched except `test/vault.test.ts`'s six `resolveForFill` call sites, which were moved to the split API) |
| 4 | `npx electron-vite build` | **clean, one rebuild** (hashes in §0) |
| 5 | `bash bench/fidelity-all.sh` | **ALL FIDELITY SCENARIOS: GREEN** — form, rerender, widgets, biglist, selects, blindfields. The change does not move the snapshot stream |
| 6 | live guards G1–G15 | **18/18 PASS.** The engine and preload were edited; these are the regression net |
| 7 | live guards G16–G28 | **PASS in all three phases** — allow 37/37 (including the added G16-seed, G25b and G26a-blind), deny 3/3, none 2/2 |
| 8 | §15.3 part 2 | recorded above; 9 live sabotages each turning its named guard and no other, 2 non-live ones recorded honestly |
...
- **The punycode origin on an IDN fixture (the second half of battery item
  ...
  unverified.
- **U2, U3, U4, U5, U6, U7** from §16 remain open exactly as the spec left them.
  U5 in particular: §5's top-frame rule is enforced (`el.ownerDocument !==
  document` → `subframe`), but no fixture has ever put a password field in a
  same-origin iframe to confirm the walk cannot see it.
- **`FIELD_IN_SUBFRAME` and `FILL_UNCONFIRMED`** have wire strings and code
  paths but no guard reaches them. The first needs U5's fixture; the second
  needs a page that hangs the preload for five seconds.
